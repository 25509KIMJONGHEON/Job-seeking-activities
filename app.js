...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initializeEventListeners() {
        ...
        dom.sortButtons.forEach(button => button.addEventListener('click', handleSortChange));
    ...
    async function fetchAndDisplayBooks() {
        ...
        try {
            const { currentKeyword, currentPage, currentSortOrder, currentCategory } = state;
            const response = await fetch(`/search?q=${currentKeyword}&page=${currentPage}&orderBy=${currentSortOrder}&category=${currentCategory}`);
            ...
            dom.resultsContainer.innerHTML = '<p>情報の取得中にエラーが発生しました。</p>';
        }
    }
    ...
    async function showMyList() {
        ...
        try {
            const bookPromises = state.userFavorites.map(bookId => fetch(`/api/book/${bookId}`).then(res => res.json()));
            ...
            dom.myListContent.innerHTML = '<p>リストの読み込み中にエラーが発生しました。</p>';
        }
    }
    ...
    async function removeFromMyList(bookId, cardElement) {
        try {
            ...
            const mainResultCard = dom.resultsContainer.querySelector(`[data-book-id="${bookId}"]`);
            if (mainResultCard) {
    ...
    function showBookDetails(bookItem) {
        ...
        textContentDiv.querySelector('.author').innerHTML = `<strong>著者:</strong> ${authors ? authors.join(', ') : '著者不明'}`;
